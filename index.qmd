...
* Idea: design a simulation where the (estimated) posterior is the same as the (simulated) prior.
...
y^{\text{sim}} &\sim p(y | \theta^{\text{sim}}) \qquad &&\text{3. Draw data given parameters.} \\
...
* Equivalent to integrating out $y^{\text{sim}}$ to get the prior back:

:::

$$
p(\theta) = \int p(\theta | y^{\text{sim}}) p(y^{\text{sim}} | \theta^{\text{sim}}) p(\theta^{\text{sim}}) d \theta^{\text{sim}}
$$
